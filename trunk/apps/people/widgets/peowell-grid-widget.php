<?php
if (!class_exists('peowell_grid_widget') && class_exists( 'WPH_Widget_Mod' )) {
	class peowell_grid_widget extends WPH_Widget_Mod{
		function __construct(){
			$args = array( 
				'slug' => 'peowell_grid_widget', 	
				'label' => __( 'People Grid (well)', 'well_textdomain' ), 						
		...
		function widget( $args, $instance ){	
			...
			$out .= do_shortcode('[well_people_grid ids="'.$instance['ids'].'" cat="'.$instance['cat'].'" limit="'.$instance['limit'].'" pagination="'.$instance['pagination'].'" column="'.$instance['column'].'" column_margin="'.$instance['column_margin'].'" filter="'.$instance['filter'].'" /]');
			echo $out;
		}
	...
	if (!function_exists('peowell_grid_register_widget')){
		function peowell_grid_register_widget(){
			register_widget( 'peowell_grid_widget' );
		}
		add_action( 'widgets_init', 'peowell_grid_register_widget', 1 );
	}
}