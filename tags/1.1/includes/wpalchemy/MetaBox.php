...
add_action('admin_head', array('WPAlchemy_MetaBox_Mod', '_global_head'));

add_action('admin_footer', array('WPAlchemy_MetaBox_Mod', '_global_foot'));

if(!defined('WPALCHEMY_MOD_MODE_ARRAY')) define('WPALCHEMY_MOD_MODE_ARRAY', 'array');

if(!defined('WPALCHEMY_MOD_MODE_EXTRACT')) define('WPALCHEMY_MOD_MODE_EXTRACT', 'extract');

if(!defined('WPALCHEMY_MOD_FIELD_HINT_TEXT')) define('WPALCHEMY_MOD_FIELD_HINT_TEXT', 'text');

if(!defined('WPALCHEMY_MOD_FIELD_HINT_TEXTAREA')) define('WPALCHEMY_MOD_FIELD_HINT_TEXTAREA', 'textarea');

if(!defined('WPALCHEMY_MOD_FIELD_HINT_CHECKBOX')) define('WPALCHEMY_MOD_FIELD_HINT_CHECKBOX', 'checkbox');

if(!defined('WPALCHEMY_MOD_FIELD_HINT_CHECKBOX_MULTI')) define('WPALCHEMY_MOD_FIELD_HINT_CHECKBOX_MULTI', 'checkbox_multi');

if(!defined('WPALCHEMY_MOD_FIELD_HINT_RADIO')) define('WPALCHEMY_MOD_FIELD_HINT_RADIO', 'radio');

if(!defined('WPALCHEMY_MOD_FIELD_HINT_SELECT')) define('WPALCHEMY_MOD_FIELD_HINT_SELECT', 'select');

if(!defined('WPALCHEMY_MOD_FIELD_HINT_SELECT_MULTI')) define('WPALCHEMY_MOD_FIELD_HINT_SELECT_MULTI', 'select_multi');

// depreciated, use WPALCHEMY_MOD_FIELD_HINT_SELECT_MULTI instead
if(!defined('WPALCHEMY_MOD_FIELD_HINT_SELECT_MULTIPLE')) define('WPALCHEMY_MOD_FIELD_HINT_SELECT_MULTIPLE', 'select_multiple');

if(!defined('WPALCHEMY_MOD_LOCK_TOP')) define('WPALCHEMY_MOD_LOCK_TOP', 'top');

if(!defined('WPALCHEMY_MOD_LOCK_BOTTOM')) define('WPALCHEMY_MOD_LOCK_BOTTOM', 'bottom');

if(!defined('WPALCHEMY_MOD_LOCK_BEFORE_POST_TITLE')) define('WPALCHEMY_MOD_LOCK_BEFORE_POST_TITLE', 'before_post_title');

if(!defined('WPALCHEMY_MOD_LOCK_AFTER_POST_TITLE')) define('WPALCHEMY_MOD_LOCK_AFTER_POST_TITLE', 'after_post_title');

if(!defined('WPALCHEMY_MOD_VIEW_START_OPENED')) define('WPALCHEMY_MOD_VIEW_START_OPENED', 'opened');

if(!defined('WPALCHEMY_MOD_VIEW_START_CLOSED')) define('WPALCHEMY_MOD_VIEW_START_CLOSED', 'closed');

if(!defined('WPALCHEMY_MOD_VIEW_ALWAYS_OPENED')) define('WPALCHEMY_MOD_VIEW_ALWAYS_OPENED', 'always_opened');

class WPAlchemy_MetaBox_Mod
{
	/**
	 ...
	 * WPALCHEMY_MOD_MODE_ARRAY (default) and WPALCHEMY_MOD_MODE_EXTRACT, this option
	 * should be used when instantiating the class.
	 *
	 * @since	1.2
	 * @access	public
	 * @var		string
	 */
	var $mode = WPALCHEMY_MOD_MODE_ARRAY;

	/**
	 * When the mode option is set to WPALCHEMY_MOD_MODE_EXTRACT, you have to take
	...
	function WPAlchemy_MetaBox_Mod($arr)
	{
		...
		if (is_array($arr))
		{
			...
			if ($this->lock_on_top) $this->lock = WPALCHEMY_MOD_LOCK_TOP;
			elseif ($this->lock_on_bottom) $this->lock = WPALCHEMY_MOD_LOCK_BOTTOM;
	...
	{
		if (WPALCHEMY_MOD_MODE_ARRAY == $this->mode AND $key == $this->id)
		{
	...
	function _init()
	{
		...
		if ( ! WPAlchemy_MetaBox_Mod::_is_post() AND ! WPAlchemy_MetaBox_Mod::_is_page()) return;
	...
	function _foot()
	{
		...
		)
		{
			...
			(function($){ /* not using jQuery ondomready, code runs right away in footer */
				...
				<?php if (WPALCHEMY_MOD_LOCK_TOP == $this->lock): ?>
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (WPALCHEMY_MOD_LOCK_BOTTOM == $this->lock): ?>
				<?php if ('side' == $this->context): ?>
				...
				<?php endif; ?>
				$('#'+id).append(mb);
				<?php elseif (WPALCHEMY_MOD_LOCK_BEFORE_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				...
				$('#'+id).append(mb);
				<?php endif; ?>
				<?php elseif (WPALCHEMY_MOD_LOCK_AFTER_POST_TITLE == $this->lock): ?>
				<?php if ('side' != $this->context): ?>
				...
				<?php if (WPALCHEMY_MOD_VIEW_START_OPENED == $this->view): ?>
				mb.removeClass('closed');
				<?php elseif (WPALCHEMY_MOD_VIEW_START_CLOSED == $this->view): ?>
				mb.addClass('closed');
				<?php elseif (WPALCHEMY_MOD_VIEW_ALWAYS_OPENED == $this->view): ?>
	...
	{
		if ('post' == WPAlchemy_MetaBox_Mod::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		if ('page' == WPAlchemy_MetaBox_Mod::_is_post_or_page())
		{
			return TRUE;
		}
	...
	{
		$post_type = WPAlchemy_MetaBox_Mod::_get_current_post_type();
	...
	static function _get_current_post_type()
	{
		...
		if ( isset( $uri ) )
		{
			...
			{
				$post_id = WPAlchemy_MetaBox_Mod::_get_post_id();
	...
	function _meta($post_id = NULL, $internal = FALSE)
	{
		...
		// WPALCHEMY_MOD_MODE_ARRAY

		$meta = get_post_meta($post_id, $this->id, TRUE);

		// WPALCHEMY_MOD_MODE_EXTRACT
	...
	{
		if (!$this->in_template AND $this->mode == WPALCHEMY_MOD_MODE_EXTRACT)
		{
		...
		$hints = array
		(
			WPALCHEMY_MOD_FIELD_HINT_CHECKBOX_MULTI,
			WPALCHEMY_MOD_FIELD_HINT_SELECT_MULTI,
			WPALCHEMY_MOD_FIELD_HINT_SELECT_MULTIPLE,
		);
	...
	function _save($post_id)
	{
		...
		WPAlchemy_MetaBox_Mod::clean($new_data);
		...
		if ($this->mode == WPALCHEMY_MOD_MODE_EXTRACT)
		{
			$new_fields = array();
			...
			// keep data tidy, delete values if previously using WPALCHEMY_MOD_MODE_ARRAY
		...
		else
		{
			...
			// keep data tidy, delete values if previously using WPALCHEMY_MOD_MODE_EXTRACT
	...
	static function clean(&$arr)
	{
		if (is_array($arr))
		{
			foreach ($arr as $i => $v)
			{
				if (is_array($arr[$i]))
				{
					WPAlchemy_MetaBox_Mod::clean($arr[$i]);
