 ...
 */
 ...
 class WPAlchemy_MediaAccess_mod
{
	/**
	...
	public function getField(array $attr)
	{
		...
		(
			'type' => 'text',
			'class' => $this->field_class_name . '-' . $groupname . ' selected-field',
		);
	 ...
	 * @see		getButton()
	 */
	public function getField_image(array $attr)
	{
		$groupname = isset($attr['groupname']) ? $attr['groupname'] : $this->groupname ;
		
		$attr_default = array
		(
			'class' => $this->field_class_name . '-' . $groupname . ' selected-image',
		);
				...
				jQuery(function($)
				{
						...
						$('[class*=<?php echo $this->button_class_name; ?>]').live('click', function(e)
						{
							...
							customMediaManager.on('select', function(){
								...
								form.find('input[type="text"].selected-field').val(media_attachment.url);
								form.find('img.selected-image').attr('src',media_attachment.url);
							});
