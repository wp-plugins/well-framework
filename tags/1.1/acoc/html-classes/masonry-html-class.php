<?php
if(!class_exists('acoc_masonry_html')):

class acoc_masonry_html{
	public $display;
	public $uid;
	public $column;
	public $margin;
	...
	function __construct($data){
		...
		$this->uid = 'acoc-masonry-uid-'.rand();
	...
	function start(){
		...
		/*$output .= '<div id="'.$this->uid.'" class="js-masonry" data-masonry-options=\'{ "columnWidth": 200, "itemSelector": ".'.$this->uid.'-item" }\'>';*/
		...
		function filter($taxonomy){
			...
			if ( $count > 0 ){
				$output .= '<ul id="'.$this->uid.'-filter" class="masonry-acoc-filter">';
		...
		function in_loop_start($class = NULL){
			$output = '<div class="'.$this->uid.'-item '.$class.' masonry-wp-child-item">';
			if($this->display == false){ return $output; }else{  echo $output; }
		}
	...
	function end(){
        ...
        <script type="text/javascript">
			jQuery(document).ready(function($){
				var $masonry_acoc_container = $('#<?php echo $this->uid; ?>');
				
				$masonry_acoc_container.imagesLoaded( function() {
					$masonry_acoc_container.masonry({
				...
				$('#<?php echo $this->uid; ?>-filter').on( 'click', 'li', function() {
					...
					$( '.<?php echo $this->uid; ?>-item' ).css( "display", "none" );
					$( '.<?php echo $this->uid; ?>-item'+filterValue ).css( "display", "block" );
					$masonry_acoc_container.masonry({
  						itemSelector: '.<?php echo $this->uid; ?>-item',
					});
