...
* @version      1.6
*/
if ( ! class_exists( 'WPH_Widget_Mod' ) ) 
{

    class WPH_Widget_Mod extends WP_Widget
    {
        ...
        */
        ...
        function create_field_textarea( $key, $out = "" )
        {
            ...
            $out .= 'id="'. esc_attr( $key['_id'] ) .'" name="' . esc_attr( $key['_name'] ) . '">'.$value;
        ...
        */
        ...
        function create_field_select( $key, $out = "" )
        {
			...
			if(isset($key['multiple']) && $key['multiple'] == 'yes'){ $out .= 'multiple="multiple"'; }
        ...
        */
        ...
        function create_field_select_tax( $key, $out = "" )
        {
            $out .= $this->create_field_label( $key['name'], $key['_id'] ) . '<br/>';
        ...
        */
        ...
        function create_field_select_typs( $key, $out = "" )
        {
            $out .= $this->create_field_label( $key['name'], $key['_id'] ) . '<br/>';
			...
			$the_query = new WP_Query( array( 'post_type' => $key['typs'], 'posts_per_page' => -1 ) );
			if ( $the_query->have_posts() ) {
                while ( $the_query->have_posts() ) {
					$the_query->the_post();

                    $out .= '<option value="' . esc_attr__( get_the_ID() ) . '" ';

                    if ( esc_attr( $selected ) == get_the_ID() )
                        $out .= ' selected="selected" ';

                    $out .= '> '.esc_html( get_the_title() ).'</option>';
