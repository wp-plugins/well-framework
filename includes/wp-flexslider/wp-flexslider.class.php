...
class wp_flexslider{
	...
	function the_css(){
		echo '<style type="text/css">';
			if(($this->itemWidth != '0') && ($this->itemMargin != '0')) { 
				echo '#'.$this->uid.' .wp-flexslider-holder .wfs-viewport ul.slides{ margin-left: -'.$this->itemMargin.'px;  }'; 
				echo '#'.$this->uid.' .wp-flexslider-holder .wfs-viewport ul.slides li{ margin-left: '.$this->itemMargin.'px; }';
			}
		echo '</style>';
	}
	...
	*/
	function content(){
		ob_start();
			$this->the_css();
			echo '<div class="wp-flexslider-clear"></div>';
			echo '<div class="wp-flexslider '.$this->div_class.'" id="'.$this->uid.'">';
				$this->main_loop();
			echo '</div>';
			echo '<div class="wp-flexslider-clear"></div>';
			$this->the_js();
	...
	*/
	function before_loop(){
		echo '<div class="wp-flexslider-holder" id="'.$this->uid.'-slideshow">';
...
* @since  1.0
*/
add_action( 'wp_enqueue_scripts', 'wp_flexslider_scripts' );
function wp_flexslider_scripts(){
	wp_register_style('wp-flexslider', wp_flexslider::path('url').'css/flexslider.css' );
	wp_enqueue_style('wp-flexslider');
	wp_register_script('jquery-flexslider', wp_flexslider::path('url').'js/jquery.flexslider-min.js', array('jquery'), '', true);	
